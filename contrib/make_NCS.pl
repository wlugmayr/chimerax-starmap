...
my $pdbfile;
my $interact_dist = 999.0;  # min interaction distance
my $primary_chain = 'A';
my @secondary_chains = ();
my $fastDistCheck = 0;
...
#if ($primary_chain eq '' && substr ((basename $pdbfile), 4, 1) =~ /[A-Z]/) {
#	$primary_chain = substr ((basename $pdbfile), 4, 1);
#}
...
foreach my $j (0..$#secondary_chains_filt) {
	...
	my $err_pos = [0,0,0];
	my $R_i = [ [1,0,0], [0,1,0], [0,0,1] ];
	foreach my $k (1..$sym_order) {
		$err_pos = vadd( $err_pos, mapply( $R_i,$newDelCOM ) );
		$R_i = mmult($newR, $R_i);
	}
	my $axis_proj_i =  [ $allQs[$i]->[0],  $allQs[$i]->[1],  $allQs[$i]->[2] ];
	...
	if (2*$sym_orders[$i] > $best_order || ( $sym_order == $best_order && $best_score > $score) ) {
		$best_order = 2*$sym_orders[$i];
		$best_score = $score;
...
foreach my $sec_chain (@best_chain) {
	...
	push @sym_orders, $sym_order;
	print STDERR "Found ".$sym_order."-fold (".(PI/$omega).") symmetric complex ".$primary_chain.":".$sec_chain_ids[0]."\n";
